...
async def inserts(os: List[_T], save: Callable[[_T], Coroutine[None, None, Tuple[ExecState, Union[Optional[_T], Exception]]]]):
    ...
    for o in os:
        ...
        if isinstance(obj, WithInfo):
            _logger.info(f'{r.name} {type(obj)}({obj.objectId})')
        else:
...
def main(args: List[str]):
    ...
    op = args[0]
    if op == 'area':
        loop.run_until_complete(crawl_areas())
        return
    if op == 'province':
        if not args[1]:
            print(HELP)
            return
        loop.run_until_complete(crawl_provinces(args[1]))
        return
    if op == 'port':
        if not args[1]:
            print(HELP)
            return
        loop.run_until_complete(crawl_ports(args[1]))
        return
    if op == 'tide':
        ...
        try:
            ...
            print(HELP)
            return
        loop.run_until_complete(crawl_tide(d, args[2]))
        return
    print(HELP)
